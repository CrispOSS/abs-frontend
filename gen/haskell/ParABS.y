...
Module :: { Module }
Module : 'module' QType ';' ListExport ListImport ListAnnotDecl MaybeBlock { Modul $2 (reverse $4) (reverse $5) (reverse $6) $7 } 
...
Decl : 'type' UIdent '=' Type ';' { TypeDecl $2 $4 } 
  ...
  | 'interface' UIdent '{' ListAnnotMethSignat '}' { InterfDecl $2 (reverse $4) }
  | 'interface' UIdent 'extends' ListQType '{' ListAnnotMethSignat '}' { ExtendsDecl $2 $4 (reverse $6) }
...
Block :: { Block }
Block : '{' ListAnnotStm '}' { Bloc (reverse $2) } 


MaybeBlock :: { MaybeBlock }
MaybeBlock : ListAnnot Block { JustBlock (reverse $1) $2 } 
  | {- empty -} { NoBlock }
...
Stm :: { Stm }
Stm : Exp ';' { SExp $1 } 
  | '{' ListAnnotStm '}' { SBlock (reverse $2) }
  | 'while' '(' PureExp ')' AnnotStm { SWhile $3 $5 }
  | 'return' Exp ';' { SReturn $2 }
  | LIdent '=' Exp ';' { SAss $1 $3 }
  | 'this' '.' LIdent '=' Exp ';' { SFieldAss $3 $5 }
  | Type LIdent ';' { SDec $1 $2 }
  | Type LIdent '=' Exp ';' { SDecAss $1 $2 $4 }
  | 'if' '(' PureExp ')' AnnotStm { SIf $3 $5 }
  | 'if' '(' PureExp ')' AnnotStm 'else' AnnotStm { SIfElse $3 $5 $7 }
  | 'suspend' ';' { SSuspend }
  | 'skip' ';' { SSkip }
...
CatchBranch : Pattern '=>' AnnotStm { CatchBranc $1 $3 } 
...
MaybeFinally : 'finally' AnnotStm { JustFinally $2 } 
  | {- empty -} { NoFinally }
...
ListAnnot :: { [Annot] }
ListAnnot : {- empty -} { [] } 
  | ListAnnot Annot { flip (:) $1 $2 }


Annot_ :: { Annot_ }
Annot_ : Type ':' PureExp { AnnWithType $1 $3 } 
  | PureExp { AnnNoType $1 }


AnnotStm :: { AnnotStm }
AnnotStm : ListAnnot Stm { AnnStm (reverse $1) $2 } 


ListAnnotStm :: { [AnnotStm] }
ListAnnotStm : {- empty -} { [] } 
  | ListAnnotStm AnnotStm { flip (:) $1 $2 }


AnnotDecl :: { AnnotDecl }
AnnotDecl : ListAnnot Decl { AnnDec (reverse $1) $2 } 


ListAnnotDecl :: { [AnnotDecl] }
ListAnnotDecl : {- empty -} { [] } 
  | ListAnnotDecl AnnotDecl { flip (:) $1 $2 }


AnnotMethSignat :: { AnnotMethSignat }
AnnotMethSignat : ListAnnot MethSignat { AnnMethSig (reverse $1) $2 } 


ListAnnotMethSignat :: { [AnnotMethSignat] }
ListAnnotMethSignat : {- empty -} { [] } 
  | ListAnnotMethSignat AnnotMethSignat ';' { flip (:) $1 $2 }
